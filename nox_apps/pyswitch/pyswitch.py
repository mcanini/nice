...
class pyswitch(Component):
    ...
    def dump_equivalent_state(self):
        ...
        for d in dpids:
            ...
            for m in macs:
                m_u = unpack("BBBBBB", m)
                di["st"][d][m_u] = utils.copy_state(self.st[d][m][0])
        return di

    def custom_copy(self, c, memo):
        c.__dict__["st"] = {}
        for dpid in self.st:
            c.st[copy.deepcopy(dpid, memo)] = {}
            for mac in self.st[dpid]:
                c.st[dpid][copy.deepcopy(mac, memo)] = copy.deepcopy(self.st[dpid][mac], memo)
